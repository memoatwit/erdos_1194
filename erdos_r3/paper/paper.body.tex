...
that survive its prefix pruning remain \texttt{UNKNOWN}. The experiment therefore
exposes a genuine cover-size versus residual-hardness tradeoff rather than a
simple speedup (§\ref{split-policy-ablation}).
...
\item
  ...
  witness-anchored cover, while measuring the increased hardness of the
  surviving cubes.
\item
...
T2 solver portfolio. The total retained-log estimate is roughly 11,230
...
but shows that its survivors are harder at the original 60-s cap; §5.3
therefore treats proof logging and survivor conquest, not raw cube count
alone, as the remaining bottleneck.
...
equally complete global-degree cover with 96,847 survivors, but its sampled
survivors are individually harder at the 60-s CP-SAT cap. The full T2 survey leaves 26/6,071
...
\path{results/global_degree_survivor_sample100_summary.json}.\par}
...
the matched ablation's three sampled global-degree survivors all time out at
60 s, so conquer cost must be measured separately from cube count.
...
\item
  ...
  96,847-cube cover, but only a targeted survivor sample has been run so far.
...
Total retained logs & $\sim11{,}230$ & excludes interactive debugging, failed encode attempts, and logs not retained locally \\
\end{longtable}
}
