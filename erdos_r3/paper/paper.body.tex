...
CaDiCaL~3.0.0 and kissat~4.0.4 both close all 20 hard chunks. The observed
deployment times differ, but the first array used heterogeneous processors;
§4.4 reports that limitation separately from the solvability result. That layered behavior is
...
native CaDiCaL~3.0.0 and kissat both close every chunk; kissat is faster on
19/20 heterogeneous-node deployment runs, a timing observation that does not
isolate solver speed. A full kissat pass over all 6,071 broad residuals closes 5,959
...
\item
  ...
  kissat~4.0.4; the initial timing data are reported as a heterogeneous-node
  deployment comparison rather than a processor-normalized speedup.
\item
...
timing comparisons are labeled accordingly.
...
All 20 paired formula hashes match. Kissat records lower wall time on 19/20
chunks, with a median paired CaDiCaL-to-kissat time ratio of 1.38. These
array tasks ran opportunistically on heterogeneous Unity CPU nodes, so the
ratio describes the deployed configurations rather than a
processor-normalized speedup. On the two
historical T1c chunks, native CaDiCaL takes 28,635.8 and 18,077.7 seconds,
while kissat takes 13,425.5 and 9,963.9 seconds. Thus the original
solvability separation does not survive the same-formula rerun; the first
array establishes only a deployment-time difference between the tested native
builds, not a processor-normalized performance separation.
...
the native runs. On the initial heterogeneous-node array, kissat records lower
wall time on 19/20 byte-identical instances; this is a deployment observation,
not a processor-normalized speedup. The optimization-form HiGHS study
...
cover audit is \path{results/global_degree_cover_independent_audit.json}.\par}
...
kissat 4.0.4 both close them on byte-identical formulas; kissat records lower
wall time on
both and supplies end-to-end certified DRAT proofs (§4.4). Lower solve or
...
\item
  ...
  measurements rather than hardware-isolated solver speedups.
\item
...
formulas. Kissat records lower wall time on 19/20, but the solvability gap disappears. The
...
Native CDCL same-formula comparison & $\sim34.0$ & CaDiCaL 3.0.0 and kissat 4.0.4 on 20 byte-identical T1b formulas \\
...
Total retained logs & $\sim11{,}324$ & excludes interactive debugging, failed encode attempts, and logs not retained locally \\
\end{longtable}
}
