...
arithmetic progression, a cardinality target --- yet, as this paper
documents, its frontier instance behaves very differently: after the
strongest problem-specific pruning we found, a small residual of subproblems
resists CP-SAT propagation, LP-relaxation-based MIP, \emph{and} CDCL under
generous wall caps. That contrast is precisely what makes the family
interesting as a benchmark: it is natural, scalable in $N$, has
verifiable ground truth at solved sizes, and exposes a failure mode shared
by all three mainstream complete-search paradigms.
...
\item
  ...
  25 HiGHS-closable T1a chunks, the 18 CDCL-closable
  T1b-minus-T1c chunks, the 2 paradigm-resistant T1c chunks, the
  6,071 broad \texttt{UNKNOWN} chunks from the 100k
...
\item
  ...
  paradigms. The pocket initially appeared invariant under CP-SAT-side
  tuning and HiGHS; after the CDCL break, it remains invariant only on the
  final two-chunk residual T1c.
\item
...
%% TODO-RESULTS: fill the six status/progress cells from
%% results/baselines/{cpsat,highs,cdcl}_monolithic_{on,off}win.json
%% (Unity job 61423761). Delete this comment when filled.
...
\bottomrule\noalign{}
\endlastfoot
CP-SAT (8 workers) & on & 24 h & TODO & TODO conflicts \\
CP-SAT (8 workers) & off & 24 h & TODO & TODO conflicts \\
HiGHS MIP (8 threads) & on & 24 h & TODO & TODO nodes, dual TODO \\
HiGHS MIP (8 threads) & off & 24 h & TODO & TODO nodes, dual TODO \\
CDCL (1 core) & on & 24 h & TODO & TODO conflicts \\
CDCL (1 core) & off & 24 h & TODO & TODO conflicts \\
\end{longtable}
}

%% TODO-RESULTS: replace the following sentence with the measured outcome.
No paradigm resolves the monolithic instance within the cap, which is the
empirical justification for the witness-split architecture of §2.2: the
campaign's progress comes from the split and the window bounds, not from
raw solver time on the whole problem.
...
the 12,582,912 AP-pruned chunks of the witness-split lattice would
require orders of magnitude more; more importantly, it is not justified by
the present evidence: the hard pocket of §4 is the binding obstacle, not
simply broad-layer throughput.
...
\subsection{A conjecture about the pocket}\label{a-conjecture-about-the-pocket}
...
\texttt{(i)} the LP relaxation upper bound on $\sum x_i$ is at most one above the
...
The combined CP-SAT, HiGHS, and CDCL evidence --- 0 \texttt{FEASIBLE}/\texttt{SAT} rows
...
the 45-instance HiGHS attack returned 0 \texttt{FEASIBLE} rows, which is
...
T1c & T1c JSONL & 2 chunks & resistant to CP-SAT, HiGHS LP-MIP, pure CDCL @ 12h, and windowed CDCL @ 4h & minimum-viable proof step \\
...
either disproves $r_3(212) ≤ 43$ (one \texttt{FEASIBLE}/\texttt{SAT} row suffices, after
...
\item
  ...
  chunks, and \texttt{drat-trim} independently verified all 18/18. The research
...
\item
  ...
  \texttt{drat-trim} verification confirmed all 18/18. The 25 HiGHS-closed T1a
  chunks and the \textasciitilde{}93,929 broad-pass CP-SAT INFEASIBLE returns remain
